...
def test_force_enable_toggle():
    compute_cfg = {
        "public": {
            "forces": {
                "anchor": {
                    "spring": {"enable": True, "k_anchor_spring": 1.0}
                }
            }
        }
    }
    names = enabled_terms(compute_cfg, phase="pre_anchor") + enabled_terms(
        compute_cfg, phase="anchor"
    )
    pmap = term_params_map(compute_cfg)
    terms = [{"name": n, "k": float(pmap[n]["k_anchor_spring"])} for n in names]
...
def test_scaling_by_k():
    ...
    cfg1 = {
        "public": {
            "forces": {"anchor": {"spring": {"enable": True, "k_anchor_spring": 1.0}}}
        }
    }
    cfg2 = {
        "public": {
            "forces": {"anchor": {"spring": {"enable": True, "k_anchor_spring": 2.0}}}
        }
    }
